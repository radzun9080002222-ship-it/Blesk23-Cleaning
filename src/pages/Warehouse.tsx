...
const statusClass = { 'не посчитано': 'badge-unknown', 'В ремонте': 'badge-repair', 'ниже эталона': 'badge-short' } as const;
function historyChange(c: StockChange | NoteChange) {
  if (isNoteChange(c)) return [`${c.name} · примечание`, `${c.before || '—'} → ${c.after || '—'}`] as const;
  return [`${c.name} · ${warehouses[c.warehouse]}`, c.removed ? `${fmt(c.before)} · удалено` : `${fmt(c.before)} → ${fmt(c.after)}`] as const;
}
...
export default function Warehouse() {
  ...
  function open(kind: Kind, item?: Item) { if(pending.current) { setError('Сначала проверьте результат предыдущей операции.'); return; } setError(''); setNotice(''); setAmounts({}); setNote(''); setItemNote(item?.note ?? ''); setConfirmDelete(false); setNewName(''); setModal({kind,item}); setRetry(false); }
  async function submit(e: React.FormEvent) {
    ...
    try {
      if (!pending.current) {
        ...
        if(modal.kind==='item') command = {kind:modal.kind,warehouse,note:note.trim(),item:{name:newName.trim(),unit:newUnit,category:newCategory,note:''}};
        else if(modal.kind==='delete') { if(!modal.item) throw new Error('Выберите позицию.'); if(!confirmDelete) throw new Error('Подтвердите удаление.'); command = deleteCommand(modal.item.id, note); }
        else if(modal.kind==='annotate') { if(!modal.item) throw new Error('Выберите позицию.'); command = annotateCommand(modal.item.id, note, itemNote); }
        else {
          ...
          command = {kind:modal.kind,warehouse,note:note.trim(),lines};
        }
  ...
  return <main className="sklad">{meta}<header className="sklad-header"><div className="brand"><span className="sklad-logo"><Package/></span><div><b>Империя Блеска</b><small>Управление складами</small></div></div><div className="header-actions"><span>{session.actor}</span><button aria-label="Выйти" onClick={async()=>{ try { await rpc('logout',{p_token:session.token}); forget(); } catch(e){fail(e);} }}><LogOut size={18}/></button></div></header><div className="sklad-content"><div className="title-row"><div><p className="eyebrow">ПОРЯДОК В ДЕТАЛЯХ</p><h1>Склад</h1><p className="muted">Знаем, что есть. Видим, чего не хватает.</p></div><div className="tools"><button onClick={refresh} disabled={busy||!!modal}><RefreshCw size={16}/> Обновить</button><button onClick={exportCsv} disabled={!data}><Download size={16}/> CSV</button><button className="primary" disabled={!data} onClick={()=>open('item')}><Plus size={17}/> Позиция</button></div></div>
      ...
      <div className="stock-table" tabIndex={0} role="region" aria-label="Остатки склада — таблица с прокруткой">
        <table className="inventory-table">
          <thead><tr><th scope="col">Наименование</th><th scope="col">Ед.</th><th scope="col">Факт</th><th scope="col" title="Желаемое количество по норме склада (фото-список эталона). Это не фактический остаток."><span className="th-stack">Эталон<small>норма, не факт</small></span></th><th scope="col">Не хватает</th><th scope="col">Статус / операции</th></tr></thead>
          <tbody>{shown.filter(i=>tab!=='purchase'||(shortage(i,warehouse)||0)>0).map(i=><tr key={i.id}>
            <th scope="row" className="item-name"><span className="name-line"><span title={i.category}>{i.name}</span>{inRepair(i.note)&&<span className="badge-repair">В ремонте</span>}{i.note&&<details className="item-note"><summary aria-label={`Примечание: ${i.name}`} title="Показать примечание">ⓘ</summary><p>{i.note}</p></details>}</span></th>
            <td className="unit-cell">{i.unit}</td>
            <td className={`number ${i[warehouse]===null?'unknown':''}`}>{fmt(i[warehouse])}</td>
            <td><button className="target-value" title="Изменить эталон — желаемое количество по норме, факт не меняется" aria-label={`Эталон: ${i.name}`} onClick={()=>open('target',i)}>{i[targetKey(warehouse)]===null?'—':fmt(i[targetKey(warehouse)])}<SlidersHorizontal size={12}/></button></td>
            <td className="number">{shortage(i,warehouse)===null?'—':<span className={shortage(i,warehouse)!>0?'badge-short':'badge-ok'}>{fmt(shortage(i,warehouse))}</span>}</td>
            <td className="status-cell"><div className="status-badges">{rowStatuses(i,warehouse).map(s=><span key={s} className={statusClass[s]}>{s}</span>)}</div><div className="row-actions"><button title="Поступление" aria-label={`Поступление: ${i.name}`} onClick={()=>open('receipt',i)}><ArrowDownToLine size={16}/></button><button title="Перемещение" aria-label={`Перемещение: ${i.name}`} onClick={()=>open('transfer',i)}><ArrowLeftRight size={16}/></button><button title="Списание" aria-label={`Списание: ${i.name}`} onClick={()=>open('writeoff',i)}><Trash2 size={16}/></button><button title="Посчитать" aria-label={`Посчитать: ${i.name}`} onClick={()=>open('inventory',i)}><ClipboardCheck size={16}/></button><button title="Примечание" aria-label={`Примечание: ${i.name}`} onClick={()=>open('annotate',i)}><Pencil size={16}/></button><button className="danger-icon" title="Удалить позицию" aria-label={`Удалить: ${i.name}`} onClick={()=>open('delete',i)}><XCircle size={16}/></button></div></td>
          </tr>)}</tbody>
        </table>
        {!shown.filter(i=>tab!=='purchase'||(shortage(i,warehouse)||0)>0).length&&<div className="empty">По выбранным условиям позиций нет.</div>}
      </div><p className="table-scroll-hint">Проведите по таблице влево, чтобы увидеть все столбцы и операции.</p></>}
    ...
    {modal.kind==='item'?<><label>Название<input autoFocus required maxLength={120} value={newName} onChange={e=>setNewName(e.target.value)}/></label><div className="form-grid"><label>Категория<select value={newCategory} onChange={e=>setNewCategory(e.target.value)}>{categories.map(c=><option key={c}>{c}</option>)}</select></label><label>Единица<select value={newUnit} onChange={e=>setNewUnit(e.target.value)}>{units.map(c=><option key={c}>{c}</option>)}</select></label></div><p className="info">Новая позиция появится с непосчитанными остатками. Укажите фактическое количество через инвентаризацию, затем оформляйте поступления.</p></>:modal.kind==='delete'?<><p className="info">Позиция «{modal.item?.name}» будет удалена из каталога безвозвратно. Факт и эталон по обоим складам исчезнут. Это не списание: остаток никуда не переносится.</p><label className="confirm-row"><input type="checkbox" checked={confirmDelete} onChange={e=>setConfirmDelete(e.target.checked)}/>Подтверждаю безвозвратное удаление</label></>:modal.kind==='annotate'?<><p className="info">Меняется только примечание. Факт и эталон остаются. Чтобы отметить ремонт, допишите «В ремонте.» — количество на строке не скрывается.</p><label>Примечание позиции<textarea aria-label="Примечание позиции" maxLength={1000} value={itemNote} onChange={e=>setItemNote(e.target.value)} placeholder="Например: В ремонте."/></label><button type="button" onClick={()=>setItemNote(withRepairNote(itemNote))}>Добавить «В ремонте.»</button></>:<><p className="info">{modal.kind==='inventory'?'Введите фактический остаток. Пустые поля не меняются; 0 означает, что позиции нет.':modal.kind==='target'?'Эталон — желаемое количество по норме склада, отдельно от факта. Фактический остаток не изменится.':'Укажите количество в единицах позиции. Для непосчитанных остатков сначала проведите инвентаризацию.'}</p><div className="inventory-lines">{(modal.item?[modal.item]:items).map(i=><label className="inventory-line" key={i.id}><span><b>{i.name}</b><small>Сейчас: {fmt(i[warehouse])} · {i.unit}{modal.kind==='transfer'?` / ${warehouses[warehouse==='adler'?'sochi':'adler']}: ${fmt(i[warehouse==='adler'?'sochi':'adler'])}`:''}</small></span><input aria-label={`Количество: ${i.name}`} inputMode="decimal" placeholder={modal.kind==='inventory'?'Факт':'Количество'} value={amounts[i.id]||''} onChange={e=>setAmounts({...amounts,[i.id]:e.target.value})}/></label>)}</div></>}
    <label>Основание / комментарий<textarea required minLength={2} maxLength={1000} placeholder="Например: закупка по чеку, передача бригаде, пересчёт склада" value={note} onChange={e=>setNote(e.target.value)}/></label></fieldset>{error&&<div className="error" role="alert">{error}{error.includes('Обновите данные')&&<button type="button" onClick={refresh}>Обновить данные</button>}</div>}{retry&&<p className="info">Результат отправки неизвестен. Повторите ту же операцию — сервер проверит, была ли она уже проведена.</p>}<div className="modal-footer"><button type="button" disabled={busy} onClick={()=>{setModal(null);setError('');}}>Закрыть</button><button className={modal.kind==='delete'?'danger':'primary'} disabled={busy||(modal.kind==='delete'&&!retry&&!confirmDelete)}>{busy?'Сохраняем…':retry?'Проверить / повторить':modal.kind==='delete'?'Удалить безвозвратно':modal.kind==='annotate'?'Сохранить примечание':'Провести операцию'}</button></div></form></section></div>}
  </main>;
}
